...
const AddCourse = () => {
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-fray text-gray-500'>
            ...
            <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
              <div className='flex justify-between items-center p-4 border-b'>
                <div className='flex items-center'>
                  <img onClick={() => handleChapter('toggle', chapter.chapterId)} src={assets.dropdown_icon} width={14} alt="" className={`mr-2 cursor-pointer transition-all ${chapter.collapsed && "-rotates-90"}`} />
                  ...
                  <div className='p-4'>
                    {chapter.chapterContent.map((lecture, lectureIndex) => (
                      <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                        <span>{lectureIndex + 1} {lecture.lectureTitle} - {lecture.lectureDuration} mins -1
                          <a href={lecture.lectureUrl} target='_blank' className='text-blue-500'>Link</a> -{lecture.isPriviewFree ? 'Free Preview' : 'Paid'} </span>

                        <img src={assets.cross_icon} alt="" onClick={() => handleLecture('remove', chapter.chapterId, lectureIndex)}
                          className='cursor-pointer' />
                      </div>
                    ))}
                    <div className='inline-flex bg-gray-100 p-2 rounded cursor-pointer' onClick={() => handleLecture('add', chapter.chapterId)}>
                      + Add Lecture
            ...
            <div className='fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center'>
              ...
              max-w-80'>
                ...
                <div className='mb-2'>
                  <p>Lecture Title</p>
                  <input type="text"
                    className='mt-1 block w-full border rounded py-1px-2'
                ...
                <div className='mb-2'>
                  ...
                  <input type="number"
                    className='mt-1 block w-full border rounded py-1px-2'
                    vlaue={lectureDetails.lectureDuration}
                ...
                <div className='mb-2'>
                  ...
                  <input type="text"
                    className='mt-1 block w-full border rounded py-1px-2'
                    vlaue={lectureDetails.lectureUrl}
                ...
                <div className='mb-2'>
                  <p>Is Preview Free?</p>
                  <input type="checkbox"
                    className='mt-1 block w-full border rounded py-1px-2 cursor-pointer'
                    vlaue={lectureDetails.isPriviewFree}
                    onChange={(e) => setLectureDetails({ ...lectureDetails, isPriviewFree: e.target.value })}
                  />
                </div>
